...
class BinaryTree {
  public:
    ...
    BinaryTree(int n, int rt, int son[][2]): root(nullptr) {
        queue<pointer> q;
        q.push(root = new node<T>(rt));
        while (!q.empty()) {
            pointer tmp = q.front();
            q.pop();
            if (son[tmp->val][0]) q.push(tmp->left = new node<T>(son[tmp->val][0]));
            if (son[tmp->val][1]) q.push(tmp->right = new node<T>(son[tmp->val][1]));
        }
    }
